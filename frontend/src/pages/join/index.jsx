...
const JoinPageForms = () => {
    const [waitingFlag] = useState(false);
    const [playerName, setPlayerName] = useState('');
    const navigate = useNavigate();
    const { addToast } = useToasts();
    const { overallPlayer } = useGame();
    ...
    const handleProcessing = () => {
        navigate('/main', { state: { name: playerName } });
    }
    return (
            ...
            <Heading />
            {
                !waitingFlag ?
                    <div className='page-content-tab-wrapper'>
                        <p className='player-status-label'>Overall Online Player: {[...overallPlayer, playerName].join(', ')}</p>
